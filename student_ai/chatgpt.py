...
class Chatbot():
    ...
    def make_message(self, msg, history):
        messages = []
    ...
    def chat(self, msg, history):
        try:
            response = self.client.chat.completions.create(
                model="gpt-4",
                messages=self.make_message(msg, history),
                temperature=0.5,
            )
    ...
    def make_test_message(self, my_ai_ans, true_ans):
        messages = []

        messages.append(
            {"role": "user", "content": f'{my_ai_ans}, 이건 내가 쓴 답이고, {true_ans}, 이건 정답이야 내가 쓴 답을 채점해서 맞으면 1 틀리면 0 을 보내줘. 모르겠어요는 틀린거야.'})
        return messages

    def test_eval(self, my_ai_ans, true_ans):
        try:
            response = self.client.chat.completions.create(
                model="gpt-4",
                messages=self.make_test_message(my_ai_ans, true_ans),
                temperature=0.5,
            )
        except Exception as ex:
            if ex.code == "insufficient_quota":
    ...
    def make_question_prompt(self, question):
        prompt = []
    ...
    def test_ai(self, question):
        try:
            response = self.client.chat.completions.create(
                model="gpt-4",
                messages=self.make_question_prompt(question),
                temperature=0.5,
            )
        except Exception as ex:
            if ex.code == "insufficient_quota":
